...
var tradeSrc = new Stream[Trade](classOf[Trade])
val vwap: Expr[Double] = simple.add(tradeSrc).map(x => x.price * x.qty)
val sumVwap = vwap.map(new Sum)

tradeSrc.value = new Trade(1.12, 100)
simple.advance()
println( "vwap: " + vwap.target.value+" sumV:"+sumVwap.target.value )
